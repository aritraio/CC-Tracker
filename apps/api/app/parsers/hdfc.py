...
class HdfcStatementParser(BaseStatementParser):
    """Deterministic parser for HDFC Bank Credit Card statements."""

    issuer_name: str = "HDFC Bank"

    def identify(self, first_page_text: str) -> bool:
        if not first_page_text:
            return False
        return bool(
            re.search(r"HDFC\s*BANK", first_page_text, re.IGNORECASE)
            or re.search(r"hdfcbank\.com", first_page_text, re.IGNORECASE)
        )
    ...
    def _extract_header(self, pages: list[str]) -> StatementHeader:
        ...
        period_match = re.search(
            r"(?:Statement\s*Period|Billing\s*Period|Statement\s*From)[:\s]*(?:From)?\s*(\d{2}[/\-\s\w]+?)\s*(?:to|-)\s*(\d{2}[/\-\s\w]+)",
            header_text,
            re.IGNORECASE,
        )
        if period_match:
            ...
            stmt_date_match = re.search(
                r"(?:Statement\s*Date|Date)[:\s]*(\d{2}[/\-][\w]+[/\-]\d{2,4})",
                header_text,
                re.IGNORECASE,
            )
            if stmt_date_match:
        ...
        credits_match = re.search(
            r"(?:Total\s*Credits|Credits|Payments\s*&\s*Credits)[:\s\D]*?([\d,]+\.\d{2}|[\d,]+)",
            header_text,
            re.IGNORECASE,
        )
        if credits_match:
    ...
    def _extract_transactions(
        ...
        txn_line_pattern = re.compile(
            r"^(\d{2}/\d{2}/\d{2,4}(?:\s+\d{2}:\d{2}(?::\d{2})?)?)\s+(.+?)\s+([\d,]+\.\d{2})\s*(Cr|Dr|CR|DR|l)?$",
            re.IGNORECASE,
        )
        ...
        for page_idx, page_text in enumerate(pages, start=1):
            ...
            for line in lines:
                ...
                if re.search(
                    r"(?:Date\s+Transaction\s+Description|Statement\s+Period|Card\s+No|Page\s+\d+|Reward\s+Points|Opening\s+Balance|Total\s+Dues|Payment\s+Due|IMPORTANT\s+INFORMATION|This\s+is\s+a\s+system\s+generated)",
                    stripped,
                    re.IGNORECASE,
                ):
                    in_txn_table = True
                ...
                if match:
                    ...
                    if transactions and in_txn_table and len(stripped) < 80:
                        if not re.search(
