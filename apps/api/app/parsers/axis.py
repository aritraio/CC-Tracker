...
class AxisStatementParser(BaseStatementParser):
    ...
    issuer_name: str = "Axis Bank"

    def identify(self, first_page_text: str) -> bool:
        if not first_page_text:
            return False
        return bool(
            re.search(r"AXIS\s*BANK", first_page_text, re.IGNORECASE)
            or re.search(r"axisbank\.com", first_page_text, re.IGNORECASE)
        )
    ...
    def _extract_header(self, pages: list[str]) -> StatementHeader:
        ...
        total_due_match = re.search(
            r"(?:Total\s*Amount\s*Due|Total\s*Dues|Total\s*Payment\s*Due)[:\s\D]*?([\d,]+\.\d{2}|[\d,]+)",
            header_text,
            re.IGNORECASE,
        )
        if total_due_match:
        ...
        min_due_match = re.search(
            r"(?:Minimum\s*Amount\s*Due|Min\s*Amount\s*Due|Minimum\s*Payment\s*Due)[:\s\D]*?([\d,]+\.\d{2}|[\d,]+)",
            header_text,
            re.IGNORECASE,
        )
        if min_due_match:
        ...
        limit_match = re.search(
            r"(?:Total\s*Credit\s*Limit|Credit\s*Limit)[:\s\D]*?([\d,]+\.\d{2}|[\d,]+)",
            header_text,
            re.IGNORECASE,
        )
        if limit_match:
    ...
    def _extract_transactions(
        ...
        txn_line_pattern = re.compile(
            r"^(\d{2}[/\-]\d{2}[/\-]\d{2,4})\s+(.+?)\s+([\d,]+\.\d{2})\s*(DR|CR|Dr|Cr)?$",
            re.IGNORECASE,
        )
        ...
        for page_idx, page_text in enumerate(pages, start=1):
            ...
            for line in lines:
                ...
                if re.search(
                    r"(?:Transaction\s+Details|Date\s+Particulars|Statement\s+Period|Card\s+No|Page\s+\d+|Total\s+Dues|Payment\s+Due|Terms\s+and\s+Conditions|GSTIN|CIN\s*No|EDGE\s*REWARDS)",
                    stripped,
                    re.IGNORECASE,
                ):
                    continue
